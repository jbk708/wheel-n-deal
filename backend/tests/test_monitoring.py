...
def test_counter_metrics():
    ...
    # Get initial values
    http_requests_initial = HTTP_REQUESTS_TOTAL.labels(
    ...
    assert (
        ...
        == http_requests_initial + 1
    )
    assert DATABASE_CONNECTIONS._value.get() == db_connections_initial + 1
    assert (
        SCRAPER_REQUESTS_TOTAL.labels(website="amazon")._value.get() == scraper_requests_initial + 1
    )
...
def test_scraper_metrics_error():
    ...
    # Get initial values
    errors_initial = SCRAPER_ERRORS_TOTAL.labels(
        website=website, error_type="exception"
    )._value.get()
    ...
    assert (
        SCRAPER_ERRORS_TOTAL.labels(website=website, error_type="exception")._value.get()
        > errors_initial
    )
